...
export async function refreshUserProgress(): Promise<{ xp: number; streak: number; hearts: number } | null> {
  ...
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return null;
  const { data, error } = await supabase.from("user_progress").select("xp, streak, hearts").eq("user_id", user.id).maybeSingle();
...
export async function addXp(amount: number): Promise<{ success: boolean; newXp: number }> {
  ...
  if (error) {
    ...
    if (progress) {
      ...
      if (updateError) { console.error("add_xp fallback update failed:", updateError.message); return { success: false, newXp: currentXp }; }
...
export async function toggleFavorite(questionId: number): Promise<{ success: boolean; isFavorite: boolean }> {
  ...
  if (existing) {
    ...
    if (error) { console.error("Failed to update favorite:", error); return { success: false, isFavorite: false }; }
  } else {
    isFavorite = true;
    ...
    if (error) { console.error("Failed to insert favorite:", error); return { success: false, isFavorite: false }; }
  }
...
export async function getFavorites() {
  ...
  if (error) { console.error("Failed to get favorites:", error); return []; }
...
export async function getMistakeRecoveryProgress() {
  ...
  const { data } = await supabase.from("user_actions").select("*").eq("user_id", user.id).eq("is_mistake", false).order("created_at", { ascending: false });
...
export async function getTodayCorrectCount(): Promise<number> {
  ...
  if (error) { console.error('Failed to get today correct count:', error); return 0; }
  return count || 0;
}
...
export async function getDailyTasks() {
  ...
  if (error) { console.error("Failed to get daily tasks:", error.message); return []; }
  if (!data || data.length === 0) {
    ...
    if (insertError) { console.error("Failed to create daily tasks:", insertError.message); return []; }
    return newTasks || [];
  }
  return data;
}
